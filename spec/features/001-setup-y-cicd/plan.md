...
Esta feature establece la infraestructura inicial para **TinderCleaner**. Debido a que la edición de código se realiza en un entorno sin Xcode local (Windows/VS Code), **GitHub Actions** actúa como el entorno exclusivo de build, lint, testing y empaquetado.

### Decisiones Arquitectónicas Principales

1. **Configuración de Target Xcode (`TinderCleaner.xcodeproj`):**
   ...
   - **Estructura Modular Nativa:** Código organizado en `/codigo/TinderCleaner` y tests en `/codigo/TinderCleanerTests`.
...
3. **Pipeline CI/CD (`.github/workflows/build.yml`):**
   ...
   - Fases secuenciales:
     ...
     6. Build Release para `iphoneos` y empaquetado en `.ipa` no firmado (`TinderCleaner-unsigned.ipa`).
     7. Publicación del artefacto `TinderCleaner-unsigned-ipa`.
...
├── TinderCleaner/
│   ├── App/
│   │   ├── TinderCleanerApp.swift       ← Entrypoint SwiftUI con @main y WindowGroup
...
- Empaquetado `Payload/TinderCleaner.app` -> `zip TinderCleaner-unsigned.ipa`.
...
| **1.4 Empaquetado IPA** | Script de packaging `.ipa` en CI | Artefacto `TinderCleaner-unsigned-ipa` disponible para descarga. |
