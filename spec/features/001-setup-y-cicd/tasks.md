...
- [x] **Task 1.1:** Crear la estructura de directorios del proyecto en `/codigo` (`TinderCleaner/App`, `TinderCleaner/Resources`, `TinderCleanerTests/InfrastructureTests`, `scripts`).
  - *Criterio de Aceptación:* Directorios creados según el plan técnico.
- [x] **Task 1.2:** Crear y configurar `TinderCleaner.xcodeproj/project.pbxproj` para target iOS 17.0, Swift 6 con `SWIFT_STRICT_CONCURRENCY = complete` y sin firma requerida.
  - *Criterio de Aceptación:* El archivo PBX define la configuración de build requerida sin errores de formato.
- [x] **Task 1.3:** Crear `TinderCleanerApp.swift` como entrypoint `@main` SwiftUI e `Info.plist` con `NSPhotoLibraryUsageDescription` exacto.
...
- [x] **Task 3.1:** Crear el archivo `NetworkGuardrailTests.swift` en `codigo/TinderCleanerTests/InfrastructureTests/`.
...
- [x] **Task 4.4:** Configurar el paso de compilación Release y empaquetado `.ipa` no firmado (`Payload/TinderCleaner.app` zipped a `TinderCleaner-unsigned.ipa`).
  - *Criterio de Aceptación:* El pipeline genera la estructura `.ipa` correcta.
- [x] **Task 4.5:** Configurar la subida del artefacto `TinderCleaner-unsigned-ipa` mediante `actions/upload-artifact@v4`.
